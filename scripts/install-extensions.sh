#!/bin/bash
# copied from https://github.com/geoserver/docker/blob/master/install-extensions.sh
...
function download_extension() {
  ...
  if [ -e "$DOWNLOAD_FILE" ]; then
    ...
    if curl --output /dev/null --silent --head --fail "${URL}"; then
        ...
        wget -e use_proxy=yes -e http_proxy=${http_proxy} -e https_proxy=${https_proxy} --progress=bar:force:noscroll -c --no-check-certificate "${URL}" -O ${DOWNLOAD_FILE}
      else
